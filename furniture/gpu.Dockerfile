...
RUN /opt/conda/condabin/conda install -n ${VENV_NAME} -c anaconda libopenblas
RUN /opt/conda/condabin/conda install -n ${VENV_NAME} pytorch=1.10 cudatoolkit=11.3 torchvision -c pytorch
...
# Install JAX
RUN /opt/conda/envs/${VENV_NAME}/bin/pip install jax flax ml_collections optax tensorflow-probability
RUN /opt/conda/condabin/conda install -n ${VENV_NAME} -c anaconda cudnn=8.2.1
